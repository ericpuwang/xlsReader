...
func (h *Header) sectorSize() (size uint32) {
	if bytes.Equal(h.MajorVersion[:], MajorVersion3) {
		size = 512
	}
	if bytes.Equal(h.MajorVersion[:], MajorVersion4) {
		size = 4096
	}
...
func (h *Header) validate() (err error) { // nolint: gocyclo
	...
	if !bytes.Equal(h.HeaderSignature[:], HeaderSignature) {
	...
	if !bytes.Equal(h.ByteOrder[:], ByteOrder) {
	...
	if bytes.Equal(h.MajorVersion[:], MajorVersion3) && !bytes.Equal(h.SectorShift[:], SectorShiftForMajorVersion3) {
		return errors.New(`If Major Version is 3, the Sector Shift MUST be 0x0009, specifying a sector size of 512 bytes`)
	}

	if bytes.Equal(h.MajorVersion[:], MajorVersion4) && !bytes.Equal(h.SectorShift[:], SectorShiftForMajorVersion4) {
		return errors.New(`If Major Version is 4, the Sector Shift MUST be 0x000C, specifying a sector size of 4,096 bytes`)
	}

	if !bytes.Equal(h.MiniSectorShift[:], MiniSectorShift) {
	...
	if !bytes.Equal(h.MiniStreamCutoffSize[:], MiniStreamCutoffSize) {
		return errors.New(`Mini Stream Cutoff Size structure MUST be set to 0x00001000`)
	}

	if bytes.Equal(h.MajorVersion[:], MajorVersion3) && !bytes.Equal(h.NumberDirectorySectors[:], NumberDirectorySectorsForMajorVersion3) {
